export function yolo(options) {
  const classes = "classes" in options ? parseInt(options['classes']) : 20;
  const total = "num" in options ? parseInt(options["num"]) : 1;
  let a = "mask" in options ? options["mask"] : 1;
  const mask = parse_yolo_mask(a);
  const max_boxes = "max" in options ? parseInt(options["max"]) : 1;
  const l = {};
  l.type = "YOLO";
