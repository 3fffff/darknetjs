...
function createAveragePoolProgramInfo(inferenceHandler, inputs, isGlobalOperator, kernelShape = [], autoPad = '', strides = [], pads = [], countIncludePad) {
  ...
  adjustPoolAttributes(isGlobalOperator, inputShape, kernelShape, strides, pads);
  const outputShape = computePoolOutputShape(isGlobalOperator, inputShape, strides, kernelShape, pads, autoPad);
...
function createMaxPoolProgramInfo(inferenceHandler, inputs, isGlobalOperator, kernelShape = [], autoPad = '', strides = [], pads = []) {
  ...
  const outputShape = PoolConvUtil.computePoolOutputShape(isGlobalOperator, inputShape, strides, kernelShape, pads, autoPad);
  const op1 = `
...
function GeneratePoolingCode(x, kernelShape, pads, strides, op1, op2, startVal) {
  ...
  if (kernelShape.length <= 2) {
    ...
    const kernelSize = getSizeFromDimensionRange(kernelShape, 0, kernelShape.length);
    const kernelStrides = WebGL.computeStrides(kernelShape);
