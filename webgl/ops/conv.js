...
class WebGLConv {
  ...
  createProgramInfos(inferenceHandler, inputs) {
    ...
    PoolConvUtil.adjustPadsBasedOnAutoPad(inputs[0].dims, this.strides, this.dilations, this.kernelShape, this.pads, this.autoPad);
    console.log('Conv', `autpPad:${this.autoPad}, dilations:${this.dilations}, group:${this.group}, kernelShape:${this.kernelShape}, pads:${this.pads}, strides:${this.strides}`);
  ...
  createRunDatas(inferenceHandler, programInfos, inputs) {
    ...
    let kTD = inferenceHandler.getTextureData(k.index);
    if (!kTD) {
      console.log('Conv', 'Did not find the adjustedKernel texture in the cache. Creating rew.');
      const newKernelData = WebGLConv.prepKernelForDotProduct(k.dims.slice(), this.group, 4, k.floatData);
